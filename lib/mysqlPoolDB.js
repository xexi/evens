    ...
    this.on('makeQuery', (r, db)=>{
      if(typeof(r.esult[r.op].customQuery)!=='undefined'){
        ...
        if(isEmpty){
          ...
          if(typeof(r.esult[r.op].customQuery(s, r.gD).query.sql)!=='undefined'){
            r.esult[r.op].query = { sql : r.esult[r.op].customQuery(s, r.gD).query.sql,
                                    timeout: r.qT };
            if(typeof(r.esult[r.op].customQuery(s, r.gD).query.timeout)!=='undefined'){
              r.esult[r.op].query.timeout = r.esult[r.op].customQuery(s, r.gD).query.timeout;
            }
          } else {
            r.esult[r.op].query = { sql : r.esult[r.op].customQuery(s, r.gD).query,
                                  timeout : r.qT };
          }
